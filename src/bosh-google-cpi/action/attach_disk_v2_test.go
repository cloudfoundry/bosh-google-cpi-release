...
var _ = Describe("AttachDisk", func() {
	...
	JustBeforeEach(func() {
		attachDisk = NewAttachDiskV2(diskService, vmService, registryClient)
	})
	...
	Describe("Run", func() {
		...
		It("attaches the disk", func() {
			...
			Expect(response).To(Equal("fake-disk-device-path"))
		})
		...
		It("attaches the disk", func() {
			...
			Expect(registryClient.UpdateCalled).To(BeTrue())
			Expect(registryClient.UpdateSettings).To(Equal(expectedAgentSettings))
			Expect(response).To(Equal(vmService.AttachDiskDevicePath))
		})
